...
>
> **Because that runtime enforcement is not yet wired, Modes 2 / 3 / 3b (identity / full / full+JIT) are preview-gated.** In **multi-tenant mode** they are **refused for tenants** unless the operator explicitly sets `SAGEWAI_SEALED_PREVIEW=1` — a tenant must never receive an unenforced Sealed identity by default. In **single-org mode** the trusted self-hosted operator may run them **at their own risk** (the gate allows them, behaviour unchanged). The gate is enforced at two choke points: the run-submit route (`POST /api/v1/workflows/enqueue`, returns `403`) and a worker backstop in `_execute_workflow` (fails the run before it executes), so non-API run paths — replay, direct enqueue — are covered too. Modes 0 / 1 (bare / sandboxed) are never preview-gated. The policy lives in `sagewai.sandbox.policy.identity_execution_allowed()`.
