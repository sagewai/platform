...
async def _seed_proposed_plan(client: AdminClient, task_id: str) -> None:
    task = _task(task_id, project_id="p")
    record = _record(task)
    events = (
        _event(task, 1, TaskEventType.TASK_CREATED, {"title": task.title}),
        ...
        _event(task, 3, TaskEventType.GATE_REQUESTED, {"gate_id": f"plan:{task_id}:1"}),
        _event(task, 4, TaskEventType.TASK_STATUS_CHANGED, {"status": "PLAN_PROPOSED"}),
    )
    await client.app.state.task_store.create(
        task, events=events, record=fold_record(record, events)
    )
...
@pytest.mark.asyncio
async def test_get_task_returns_the_proposed_plan_before_acceptance(
    client: AdminClient,
) -> None:
    await _seed_proposed_plan(client, "t-proposed")

    response = await client.http.get("/api/v1/tasks/t-proposed", headers=client.headers)
...
@pytest.mark.asyncio
async def test_an_accepted_plan_has_no_separate_proposal(client: AdminClient) -> None:
    await _seed_accepted_plan(client, "t-plan")

    body = (await client.http.get("/api/v1/tasks/t-plan", headers=client.headers)).json()
