...
"""Admin v2 gRPC server-streaming subscription-management routes.

Routes mounted at ``/api/v1/admin/connections/grpc/`` (mirrors the MQTT
routes from PR #375):
...
def client(sf: AdminStateFile, token: str, master_key: str) -> TestClient:
    ...
    grpc_module._test_inject_context(None)
    app = FastAPI()
...
def _create_grpc_connection(client: TestClient) -> str:
    resp = client.post(
        "/api/v1/admin/connections/",
        json={
            "protocol": "grpc",
            "display_name": "inventory-svc",
            "tags": ["catalog"],
            "credentials_backend": {"kind": "local"},
            "protocol_data": {"target": "inventory.internal:443", "tls": "tls"},
        },
    )
...
def test_subscribe_route(client: TestClient):
    ...
    with patch(
        ...
        resp = client.post(
            ...
            json={"method": "echo.EchoService/EchoStream", "request": {"x": 1}},
        )
    assert resp.status_code == 200, resp.text
    ...
    assert kwargs["spec"]["request"] == {"x": 1}
...
def test_subscribe_route_unknown_connection(client: TestClient):
    ...
    with patch(
        ...
        resp = client.post(
            "/api/v1/admin/connections/grpc/nope/subscribe",
            json={"method": "x/y"},
        )
...
def test_subscribe_route_rejects_pause(client: TestClient):
    ...
    with patch(
        ...
        resp = client.post(
            f"/api/v1/admin/connections/grpc/{conn_id}/subscribe",
            json={"method": "x/y", "overflow_policy": "pause"},
        )
    assert resp.status_code == 422


def test_subscribe_route_rejects_empty_method(client: TestClient):
    conn_id = _create_grpc_connection(client)
    fake_mgr = MagicMock()
    fake_mgr.subscribe = AsyncMock(return_value="sub-abc")
    with patch(
...
def test_subscribe_route_limit_exceeded(client: TestClient):
    conn_id = _create_grpc_connection(client)
    fake_mgr = MagicMock()
    fake_mgr.subscribe = AsyncMock(
        side_effect=SubscriptionLimitExceededError(limit=64)
    )
    with patch(
        "sagewai.connections.protocols.grpc.get_subscription_manager",
        return_value=fake_mgr,
...
def test_subscribe_route_wrong_protocol(client: TestClient):
    # Create an http connection, then try the grpc subscribe route on it.
    resp = client.post(
        "/api/v1/admin/connections/",
        json={
            "protocol": "http",
    ...
    with patch(
        ...
        resp = client.post(
            f"/api/v1/admin/connections/grpc/{http_id}/subscribe",
            json={"method": "x/y"},
        )
...
def test_drain_route(client: TestClient):
    dr = DrainResult(events=[{"message": "a"}], returned=1, remaining=3,
                     overflow_dropped=2, oversized_dropped=0,
    ...
    with patch(
        ...
        resp = client.post(
            "/api/v1/admin/connections/grpc/subscriptions/sub-abc/drain",
            json={"max_events": 25},
        )
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_drain_route_not_found(client: TestClient):
    fake_mgr = MagicMock()
    fake_mgr.drain = AsyncMock(
        side_effect=SubscriptionNotFoundError(subscription_id="x")
    )
    with patch(
        "sagewai.connections.protocols.grpc.get_subscription_manager",
        return_value=fake_mgr,
        ...
        resp = client.post(
            "/api/v1/admin/connections/grpc/subscriptions/x/drain",
            json={},
        )
    assert resp.status_code == 404
...
def test_unsubscribe_route(client: TestClient):
    ...
    with patch(
        ...
        resp = client.delete("/api/v1/admin/connections/grpc/subscriptions/sub-abc")
...
def test_unsubscribe_route_not_found(client: TestClient):
    fake_mgr = MagicMock()
    fake_mgr.unsubscribe = AsyncMock(
        side_effect=SubscriptionNotFoundError(subscription_id="x")
    )
    with patch(
        "sagewai.connections.protocols.grpc.get_subscription_manager",
        ...
        resp = client.delete("/api/v1/admin/connections/grpc/subscriptions/x")
    assert resp.status_code == 404
...
def test_list_subscriptions_route(client: TestClient):
    fake_mgr = MagicMock()
    fake_mgr.list_subscriptions = MagicMock(
        return_value=[_stats("sub-1"), _stats("sub-2")]
    )
    with patch(
        "sagewai.connections.protocols.grpc.get_subscription_manager",
        return_value=fake_mgr,
    ):
        resp = client.get("/api/v1/admin/connections/grpc/subscriptions")
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert len(body) == 2
...
def test_get_subscription_stats_route(client: TestClient):
    ...
    with patch(
        ...
        resp = client.get("/api/v1/admin/connections/grpc/subscriptions/sub-7")
    assert resp.status_code == 200, resp.text
...
def test_get_subscription_stats_not_found(client: TestClient):
    ...
    with patch(
        ...
        resp = client.get("/api/v1/admin/connections/grpc/subscriptions/x")
    assert resp.status_code == 404
