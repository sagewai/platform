...
2. Resolves the Identity cascade IF the mode needs customer credentials. Persists key NAMES on the run row. Never sees plaintext values.
...
4. Runs the agent loop appropriate to the mode:
   ...
   - **+ identity** — + customer keys in sandbox env (Tier-2 keys, behavior knobs)
   - **+ CLI agent** — + Claude Code / Codex / … (CLI agent runtime + creds)
   - **+ JIT** — + bidirectional callback for just-in-time credentials
