...
export const metadata = {
  title: 'Security tiers — Tier-1 orchestration vs Tier-2 customer credentials',
  description:
    'Two distinct LLM-key surfaces, two trust boundaries, two lifetimes. The credential model that keeps a customer-key leak from compromising the platform.',
  alternates: { canonical: 'https://docs.sagewai.ai/docs/architecture/security-tiers' },
};

<TechArticleJsonLd
  name="Sagewai security tiers"
  description="Tier-1 (orchestration) vs Tier-2 (customer-scoped) credential surfaces in Sagewai — two trust boundaries, two lifetimes."
...
Sagewai has two distinct LLM-key surfaces with different lifetimes,
sources, and trust boundaries. This page fixes which credentials live
where, who can read them, and who never sees them. Tier 1 is the
operator's orchestration brain — the keys the Sagewai Agent uses to
plan and dispatch on the worker host. Tier 2 is the customer's
identity — the keys CLI agents and tools inside the sandbox use to
do the actual user-facing work. The two tiers are governed by
different subsystems, audited differently, and must never be mixed.
A security team reviewing Sagewai should be able to read this page
and quote it back as the credential contract.
...
**Sealed coverage:** none. Sealed (the Sagewai credential subsystem;
manages per-customer secrets end-to-end with audit and revocation)
manages Tier-2; Tier-1 is the operator's plain infra concern.
...
**Lifetime:** short-lived; injected per sandbox instance, scrubbed on release via the per-run secret cleanup hook.
...
**Sealed coverage:** full. The whole stack governs this tier:

- the foundation profile-management subsystem (Sealed-i)
- external profile backends (Sealed-ii) — Vault, 1Password, AWS
  Secrets Manager, SOPS, Bitwarden
- the revocation API (Sealed-iii.A)
- the redaction layer (Sealed-iii.B)
- per-key access control (Sealed-iii.D)
- the just-in-time callback channel (Sealed-iv)
...
1. **Tier-2 plaintext never crosses the worker host process boundary.** The `SecretProvider.env_for(...)` returns env that is set on container start by the sandbox backend's native env-injection primitive. It is never logged, never written to postgres in plaintext (even via the foundation subsystem's encrypted profile file, only the encrypted form is on disk).
...
3. **Mid-run revocation works.** Through revocation: an operator can revoke a `(profile_id, secret_key)` pair; future enqueues fail-closed; in-flight runs that already injected the value get aborted (hard-revoke) or silently expire on next sandbox-start (soft-revoke).
...
5. **Pool reuse is safe.** The per-run secret cleanup hook scrubs Tier-2 env on release. If cleanup fails, the sandbox is discarded, not pooled.
...
3. **LLM provider trust.** When a CLI agent calls Anthropic / OpenAI / etc., that provider receives the prompt content. If the prompt contains secrets (because a redaction rule was missing in the redaction layer), the provider sees them. Sagewai cannot retroactively unsee.
...
## How a Tier-2 secret actually flows
...
3. **Logging plaintext secret values.** `logger.info(f"calling api with {api_key}")` is forbidden anywhere in the codebase. Audit events log key NAMES; redaction scrubs values from prompts/outputs.
...
5. **Trusting the LLM's "I'll keep it secret".** Don't ask a model to "be careful with this API key" and then put the key in the prompt. Either redact (via the redaction layer) or don't include.
...
- [`docs/architecture/security-tiers.md`](https://github.com/sagewai/platform/blob/main/docs/architecture/security-tiers.md) (canonical reference)
