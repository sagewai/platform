...
```mermaid
flowchart TD
    CP["<b>CONTROL PLANE</b> (admin server)<br/>Sees: Tier-1 NEVER. Tier-2 NEVER (just key NAMES, via audit).<br/>Postgres rows know: profile_ref + effective_*_keys (NAMES)."]
    ...
    SB["<b>SANDBOX</b> (sandboxed modes only)<br/>Sees: Tier-2 only, in os.environ.<br/>Tool runner + CLI agents read os.environ for their LLM keys + creds.<br/>No Tier-1 access (sandbox env is wiped of host vars at start)."]
    LLM["<b>LLM INFERENCE POINT</b> (external)<br/>Sees: prompts + tool schemas only.<br/>Never sees: secret VALUES — unless an agent embeds one in a prompt,<br/>in which case the redaction layer catches it before egress."]
    CP ~~~ WH ~~~ SB ~~~ LLM
```
