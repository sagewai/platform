...
export const metadata = {
  title: 'What Sagewai keeps, adapts, freezes, and retires',
  description:
    'The product-center boundary for Sagewai: which subsystems the Work coordinator is built on, which stay as lower-level capabilities, and what it takes to change one of those decisions.',
  alternates: {
    canonical: 'https://docs.sagewai.ai/docs/architecture/work-control-plane-evolution',
  },
};

<TechArticleJsonLd
  name="Sagewai product-center boundary"
  description="Which Sagewai subsystems are kept, adapted, frozen, or retired as a product surface, and the evidence required to change a decision."
  path="/docs/architecture/work-control-plane-evolution"
  articleSection="Architecture"
/>

# What we keep, adapt, freeze, and retire

Sagewai has more subsystems than any one product story needs. This page says which of them the
Work coordinator is actually built on, which remain available as lower-level capabilities, and
which have stopped being a product surface — so you can tell a supported path from a parked
one before you build on it.

Nothing here is a deprecation schedule. Git history is the archive, a classification promises
no backward compatibility, and none of it authorises mass deletion.

## The four classifications

- **Keep** — already supports the Work control plane, or is a useful capability with its own
  tests.
- **Adapt** — reuse it and add the smallest Work-facing surface required; do not rewrite it.
- **Freeze** — the code and its tests stay, but it is not extended as an alternate product
  centre without new evidence.
- **Retire as a product surface** — the narrative and the entry points go once their direct
  consumers and a replacement are checked. The code may well stay.
...
A decision moves only on one of four kinds of evidence: a current WorkContract requires the
subsystem; a reproducible runtime or retrieval failure identifies the minimum missing
mechanism; repository consumers and tests prove a retained surface is obsolete; or an accepted
security or correctness finding requires a boundary repair.

Tool availability, credentials, hypothetical future consumers, and convenience are not
evidence.
