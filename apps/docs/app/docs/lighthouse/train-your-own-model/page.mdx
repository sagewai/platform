...
- A Sagewai project with at least one running agent (so there's traffic to capture). The [first agent guide](/docs/guides/first-agent) gets you there in 10 minutes.
...
- [Inference deployment](/docs/lighthouse/inference-deployment) — the five inference tiers in detail, with a decision table for when to pick which.
- [Observability and cost](/docs/lighthouse/observability-and-cost) — quantify the $/call delta after the local model is live.
