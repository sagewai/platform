...
This page shows you how to build a content-moderation or classification surface with SageWai: a small ensemble of pre-trained transformer classifiers running inside a sealed sandbox, with a cheap LLM acting as the judge for boundary cases. After reading, you'll know how to run the reference example, how to adapt the same pattern to triage, lead-qualification, or helpdesk routing, and what costs and audit trail to expect.

It's written for engineers adding moderation, triage, or classification to a SaaS product who care about cost, audit, and not sending user content to third-party APIs.

## Before you start

- Python 3.10+ and `pip install sagewai`.
- Local-only path: [Ollama](https://ollama.com) running with a chat model (`ollama run llama3.2`).
- Cloud path (optional): an `ANTHROPIC_API_KEY` for the LLM judge, and a `VASTAI_API_KEY` if you want to run the classifiers on a rented GPU.
- About 500 MB of disk for the three HuggingFace classifiers on first run.

## How the pattern works

Three pre-trained classifiers vote on each input. A small LLM agent reads the structured votes, applies your policy, and produces a final verdict with a written reason. Every call is recorded — per-classifier verdicts, per-tool latency and cost, and the LLM's reasoning sentence — so a moderator can click any flagged post and see why.
...
Together they're under 500 MB and run on laptop CPU for development.
...
The example downloads the three classifiers on first run (about 500 MB), runs them on a small set of representative posts, and dispatches the final judgement to local Ollama. No third-party API is called; the post text never leaves the machine.
...
The classifiers run on a Vast.ai GPU pod; the judge stays on Haiku to keep the per-call cost low. Cost per moderated post lands in the sub-cent range.

### Triage variant (single LLM, strict JSON)

If you don't need a classifier ensemble — for example, support-ticket triage where one LLM call returning `{tier, reason, draft}` is enough — run the triage example instead:
...
The same shape — *classifiers in a sealed boundary, surfaced as MCP tools, judged by a cheap LLM, audit per call* — covers a wide range of routing and classification jobs. Five recipes:

### 1. Community moderation for a SaaS forum

You run the community surface for a developer-tools SaaS. ~800 posts a day. A part-time moderator reads every one today.

| Concern | How the pattern solves it |
|---|---|
| Posts must not leave the boundary (privacy, GDPR) | Classifiers run in a sealed container; the LLM judge is local Ollama; the post text never reaches a third-party API |
...
| Qualified leads need a response in under a minute | Sub-10s p50 on Haiku, sub-1s on local llama3.2 — your latency budget is your SLA evidence |
...
| Concern | How the pattern solves it |
|---|---|
| Maintainer time gets eaten re-asking for repro | The auto-respond pile asks for repro politely in your voice; you only see issues with repro already in hand |
| Keep the human in the loop on judgement calls | P0/P1 always escalate; the agent does the boring 80% |
| Don't pay anything for OSS tooling | The Ollama path is $0/month |
...
- **The cheap path is real.** Ollama as the judge model means user content never crosses your tenant boundary.

## What you're responsible for

- **Choose where the judge runs.** Local Ollama keeps content in-house but is slower and quality-bounded by your local model. A hosted model is faster but means content leaves your boundary — confirm that's acceptable for your data class.
- **Keep the policy in the system prompt under review.** The tier semantics ("never auto-respond to P0/P1", "reclaimed language is not a flag", etc.) live in your prompt; treat them like any other policy artifact and version-control them.
- **Calibrate the test suite.** Add adversarial cases from your own surface — sarcasm, in-group/out-group framing, reclaimed slurs — and re-run when you swap models.
- **Decide what gets human review.** The agent's `queue for human` verdict only helps if a human actually reads the queue; staff that loop.

## See also

- [SDK overview](/docs/pillars/sdk) — `UniversalAgent`, tools, MCP, and directives.
- [Production multitenancy](/docs/lighthouse/production-multitenancy) — how sealed profiles and per-run identity work for tenant isolation.
- [Inference deployment](/docs/lighthouse/inference-deployment) — bringing your own GPU or hosted endpoint, used by the cloud path above.
