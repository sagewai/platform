...
> - **`X-Project-ID` is an organizational filter, not tenant isolation.** Any caller who can reach the API can supply any project ID. Do not use it as a security boundary between untrusted tenants.
...
>
> Durable Postgres-backed stores for fleet, tasks, and harness, plus proper multi-tenant isolation, are on the roadmap.
