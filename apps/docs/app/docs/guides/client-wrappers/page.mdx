...
**Tier 1** wrappers are fully implemented, tested, and published to their package registries. **Tier 2** wrappers follow the same API surface and are in development — install commands will work once those packages are published. **Mobile** wrappers share their base language (TypeScript for React Native, Dart for Flutter).
