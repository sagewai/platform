...
export const metadata = {
  title: 'Observatory — cost, telemetry, and live ops dashboards',
  description:
    'Track AI agent costs, latency, and runs in real time with OpenTelemetry, VictoriaMetrics, Grafana, and the mission-control HUD. Self-hosted, no Sagewai-tier needed.',
  ...
  openGraph: {
    ...
    description:
      'Two surfaces, one OTel pipeline. Mission-control HUD for live ops, Grafana for SRE work. Reproduce every screenshot in three minutes.',
...
<HowToJsonLd
  name="Set up Sagewai Observatory for cost and telemetry dashboards"
  description="Bring up an OpenTelemetry + VictoriaMetrics + Grafana stack and a mission-control HUD to track agent cost, latency, and runs in three minutes."
  ...
  steps={[
    ...
    },
    {
      ...
      text: 'Open http://localhost:3000 for Grafana, /hud-ironman in the admin frontend for the mission-control HUD. Both read the same telemetry stream.',
    },
  ]}
/>
...
Cost tracking, OpenTelemetry pipeline, mission-control HUD, Grafana board, audit log. Observatory answers the question *"why did the AI bill quadruple?"* with a live, per-tenant breakdown you can read directly.

Two dashboards read the same telemetry stream so they never disagree: the **mission-control HUD** for live operations (agents on a graph, missions in flight, fleet posture across the top) and the **Grafana board** for analytics, finance, and historical query.
...
**Prerequisites:** Docker, the Sagewai admin backend, and an API key. See [getting started](/docs/get-started/quickstart).
...
flowchart LR
    subgraph App["Sagewai admin backend"]
        A["FastAPI routes<br/>(160+ endpoints)"]
    ...
    subgraph Surfaces["Dashboards"]
        HUD["Mission-control HUD<br/>/hud-ironman"]
        GR["Grafana<br/>:3000"]
    end
...
## Bring the stack up and drive load
...
<img
  ...
  alt="Mission-control HUD — full canvas in LIVE mode"
...
|---|---|
| Demo Sagewai to a non-engineer (CEO, customer, all-hands) | Mission-control HUD |
| Investigate a slow request or a 5xx spike | Grafana Dashboard |
| Check whether a specific project's run is making progress | Mission-control HUD — switch the project bar |
...
teams already pay for Datadog, Grafana Cloud, or run their own Prometheus/Loki. The OTel collector is the
...
- **[Fleet](/docs/platform/fleet)** — per-worker telemetry that Observatory aggregates.
- **[Security overview](/docs/platform/security)** — the redaction layer that makes telemetry safe to export.
- **[Cost management guide](/docs/guides/cost-management)** — how budget caps, cost meters, and per-team attribution are wired in the SDK.
- **[Fleet enterprise architecture](/docs/guides/fleet-architecture)** — the multi-tenant story the dashboards are scoped to.
...
- [Example 34 — Observatory cost tracking](https://github.com/sagewai/platform/blob/main/packages/sdk/sagewai/examples/34_observatory_cost_tracking.py) — per-tenant cost tracking.
