...
State a goal; the coordinator turns it into one durable Task, asks what it must, plans a cycle of Work, runs it through the operators, and stops at the gates you kept. The board, the thread and the decisions inbox are in the admin console; `sagewai task` is the same surface from a terminal. See the [quickstart](/docs/get-started/quickstart).
