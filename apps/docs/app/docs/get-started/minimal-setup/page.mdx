...
This is the minimal Sagewai setup — a single SDK process making
Tier-1 LLM calls with no sandbox and no per-customer credentials.
Useful for local prototyping, single-tenant tools, and notebook-style
exploration.

If you want the full **build-something-real** experience — Sagewai
orchestrating Claude Code inside an isolated sandbox to build a
portfolio site — go to the [Quickstart](/docs/get-started/quickstart)
instead.

**What's different here:** no worker fleet, no sandbox, no Sealed
Identity. Everything runs in one Python process using your operator
LLM keys (Tier-1). See [Architecture: Execution modes](/docs/architecture/execution-modes#mode-0--bare)
...
**Sagewai Cloud:** Purchase and activate your account at [sagewai.ai](https://sagewai.ai), then manage your agents through the cloud admin panel at [cloud.sagewai.ai](https://cloud.sagewai.ai). No infrastructure to manage — just connect and start building.
