...
export const metadata = {
  title: 'Security overview — defense-in-depth for AI agents',
  description:
...
Most agent platforms can't answer the question your security review will eventually ask: *what happens when the model gets prompt-injected and tries to exfiltrate the customer's API keys?* Sagewai is built so the answer is: it can't reach them.

This page walks a security reviewer, CISO, or compliance lead through Sagewai's security layers — what each one protects, how to enable it, and where to read the configuration details.
...
Configure: it's on by default; audit events label any redaction match. See the [Sealed redaction runbook](/docs/runbooks/sealed-redaction) for tuning.
...
- [Sealed redaction runbook](/docs/runbooks/sealed-redaction) — tuning the redaction layer.
- [PII protection guide](/docs/guides/pii-protection) — content-side filtering.

---

*Sagewai is licensed under AGPL-3.0-or-later, with commercial dual licensing for organisations that need an alternative. Enterprise tier includes SOC 2 / ISO 27001 audit export and self-hosted Sagewai-LLM in customer VPC.*
