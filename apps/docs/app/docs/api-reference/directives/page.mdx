...
`@transform` is a **first-class built-in directive** wired into the directive core — it is not registered through `DirectiveRegistry`. Its syntax uses an extended multi-argument form that the custom-directive registry does not support: the first argument is a **bare operation token** (not a quoted string), the second is a **nested directive reference** (`@context(...)`, `@memory(...)`) or a literal string, and any additional `key=value` pairs are forwarded as operation params.
...
| custom | Any async callable registered via `transform.register("name", fn)`. |
...
Register an async callable in-process; `@transform` then invokes it by name:
...
transform.register("extract_clauses", extract_clauses)
```
...
**`transform.register` vs `DirectiveRegistry`:** `transform.register("name", fn)` adds a callable to the `TransformRegistry` — it extends what `@transform` can *do*, not what directives the engine parses. To add a new directive sigil (e.g. `@kb('query')` with the single-quoted-arg form), use `DirectiveRegistry` instead.
...
All operations run under a configurable timeout (the engine's `resolution_timeout`, default `10.0` s). Uses `asyncio.wait_for` internally — Python 3.10-safe.
...
| Transform | `@transform(op, source)` | Built-in core directive (extended syntax); apply a named transform before the LLM call |
