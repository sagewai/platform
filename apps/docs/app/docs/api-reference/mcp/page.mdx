...
The Model Context Protocol (MCP) is an open standard for connecting AI agents to external tools and data sources. Sagewai includes both an MCP client (to discover and use tools from MCP servers) and MCP server hosting (to expose agents as MCP tools).
...
+----------------+       MCP Protocol       +----------------+
|   Sagewai      | <---------------------> |   MCP Server   |
|   Agent        |   (stdio / SSE / HTTP)   |   (tools)      |
|                |                          |                |
|  McpClient     |  -- tools/list -->       |  Stripe tools  |
|                |  <-- tool specs --       |  Email tools   |
|                |  -- tools/call -->       |  Calendar      |
|                |  <-- result --           |  Documents     |
+----------------+                          +----------------+
```
...
Expose Sagewai agents as MCP servers so other tools and agents can discover and use them.
...
MCP tools use the JSON Schema format for parameter definitions. When Sagewai discovers tools from an MCP server, each tool is converted to a `ToolSpec`:
...
The Sagecurator monorepo includes 9 pre-built MCP servers:
...
3. **Combine tools from multiple servers** — Agents work best with focused, domain-specific tool sets.

4. **Set timeouts for remote servers** — Use `connect_sse(url, timeout=30)` to avoid hanging on unresponsive servers.

5. **Monitor tool usage** — Use the analytics API to track which tools are called most frequently and their success rates.
