export const metadata = {
  title: 'Self-learning API — five mechanisms for improving agents',
  description:
    'APIs for agents that improve from interaction data. Five mechanisms you can use independently or together.',
  alternates: { canonical: 'https://docs.sagewai.ai/docs/api-reference/self-learning' },
};

# Self-Learning API Reference

APIs for building agents that improve from interaction data. Sagewai's self-learning layer is composed of five mechanisms that you can use independently or together.
...
`MemoryBridge` connects conversation history to the context engine. After each turn it extracts facts and stores them, making them available for retrieval in future turns or sessions.
...
Runs background consolidation over stored memory: deduplicates semantically similar facts, applies exponential time decay, and detects contradictions.
...
Incrementally builds a knowledge graph from conversation history. Entities and relations extracted from each message are upserted into NebulaGraph.
...
Create the `ContextEngine` first, then wrap it with a `MemoryBridge` to enable automatic fact extraction and storage from conversations.
...
See [Example 19](/docs/get-started/first-agent) for a complete runnable self-learning agent.
