export const metadata = {
  title: 'Workflows API — durable execution with PostgreSQL checkpointing',
  description:
    'Build Temporal-like durable workflows with Python decorators. Checkpointing, distributed workers, DLQ, load balancing. Crash-safe agentic execution.',
  alternates: { canonical: 'https://docs.sagewai.ai/docs/api-reference/workflows' },
};

# Workflows

Step-level checkpointing, distributed workers, dead letter queues, and load balancing — all expressed as Python decorators. Workflows survive process restarts and resume from the last successful checkpoint.
