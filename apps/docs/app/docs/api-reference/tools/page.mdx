export const metadata = {
  title: 'Tools & MCP API — define agent tools the right way',
  description:
    'Define Sagewai agent tools with @tool, connect to MCP servers, use the connector framework for managed integrations. Function-call discovery for any LLM.',
  alternates: { canonical: 'https://docs.sagewai.ai/docs/api-reference/tools' },
};

# Tools & MCP

Tools give agents a way to interact with external systems. Define them with the `@tool` decorator, discover them from MCP servers, or use the connector framework for managed integrations.
...
Converts a typed Python function into a `ToolSpec` with an auto-generated JSON Schema. The name, description, and parameters are extracted from the function signature and docstring.
...
Manual tool definition for cases where you need full control over the JSON Schema.
...
Connect to MCP servers and discover their tools as `ToolSpec` instances.
...
Central registry for managed connectors. It handles credential resolution, connection lifecycle, and health monitoring.
