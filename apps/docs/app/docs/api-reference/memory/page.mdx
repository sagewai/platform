...
`ContextEngine` is the unified memory layer for agents. It handles document ingestion, scoped retrieval, deduplication, and lifecycle management. Pass it as `memory=` to any agent to enable RAG without changing agent code.
