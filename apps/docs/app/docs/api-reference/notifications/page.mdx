...
Send notifications via email, Slack, or in-app channels. Channels can be configured system-wide or per project; routing rules map trigger types to the appropriate channel.
...
Central notification dispatcher. Routes each notification to the right channels based on trigger type and project context.
...
Abstract base class for notification channels. Implement `send()` to add a custom channel.
...
Sends notifications to Slack via an incoming webhook using Block Kit messages.
...
Publishes notifications to an in-app event bus or callback. Used for real-time SSE-based alerts in the admin UI. Always returns `True` (best-effort delivery).
