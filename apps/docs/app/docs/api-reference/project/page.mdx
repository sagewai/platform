export const metadata = {
  title: 'Project & Errors API — multi-tenant isolation and error handling',
  description:
    'Per-project isolation scopes runs, audit logs, rate limits, memory. Structured error hierarchy across the SDK. Multi-tenant Sagewai by default.',
  alternates: { canonical: 'https://docs.sagewai.ai/docs/api-reference/project' },
};

# Project & Errors

Project isolation scopes all agent activity — workflow runs, audit logs, rate limits, and memory — to a namespace. The error hierarchy gives you structured exception handling across the full SDK.
...
All SDK exceptions inherit from `SagewaiError`. Catch it for any SDK failure, or catch a subclass for fine-grained handling.
