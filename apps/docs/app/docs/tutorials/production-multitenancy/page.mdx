...
This walkthrough proves what you ship when you put Sagewai in front of multiple tenants' credentials: a healthcare worker physically cannot see finance tasks, vendor credentials are injected into a containerised agent at sandbox-start and scrubbed on release, and every credential resolution and pool reset is recorded in the audit log.

If you're a security reviewer evaluating Sagewai for production, or an engineer building the first multi-tenant feature on top of it, this is the page that maps the feature set to the controls your security team will ask about.

## Five guarantees this walkthrough exercises

1. **The boundary is real, not policy.** Tenant credentials live in encrypted Sealed profiles. They're injected into the sandbox env at sandbox-start and scrubbed on release. The worker host process never sees them.
2. **Fleet dispatch enforces scope at the boundary.** Workers register with `project_id` in their capability labels; the dispatcher matches tasks against capability labels. A healthcare worker is unreachable from a finance task. Example 26 ships the dispatch matcher; Example 33 ships the full integration.
...
The script seeds two tenants (healthcare, finance), registers four workers (two per tenant) with scoped capability labels, enqueues mixed-tenant tasks, and proves the dispatcher refuses cross-tenant claims. The output prints the cross-tenant attempt being rejected at the dispatch boundary.
...
Runs an agent inside a sandbox container with credentials provided by the Sealed Identity layer, calls a third-party API, and proves the credential bytes are unreachable from the worker host. The `_smoke_test_credential_leak` block confirms the scrub.

### Agent governance (approval flow + audit)
...
The pattern in this walkthrough — *Sealed profiles per tenant, capability-scoped Fleet workers, sandbox credential injection at run time, audit at the boundary* — is what a senior engineer at a 50-500-person SaaS reaches for when their first multi-tenant feature lands and the security review starts. Five domains:
...
### 4. Hybrid SaaS with on-prem customers
...
### 5. Internal multi-team platform
...
- **Primary product:** [Fleet](/docs/platform/fleet) — workers, dispatch, scoped routing.
- **Security model:** [Security overview](/docs/platform/security) — the cross-cutting credential, redaction, replay, and audit story.
- **Sibling lighthouse:** [Observability and cost](/docs/tutorials/observability-and-cost) — the audit and per-tenant cost rollup that pairs with this story.
- **Sibling lighthouse:** [Moderation and classification](/docs/tutorials/moderation-and-classification) — the same boundary applied to a different workload.
