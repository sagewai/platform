...
State a goal in plain English; Sagewai designs the agent graph, instantiates the mission, and runs it. As missions complete, the Curator captures successful answers as training data — closing the loop with the [Training Loop](/docs/pillars/training-loop) product when the dataset crosses a threshold and the local model takes over.
...
- **[Autopilot overview](/docs/autopilot)** — the user-facing introduction to the SDK surface.
- **[Admin Panel guide](/docs/guides/admin-panel)** — the operator surface where missions, blueprints, and goals live.

## See also

- **[Training Loop](/docs/pillars/training-loop)** — the loop Autopilot closes when the captured corpus crosses the threshold.
- **[Security overview](/docs/security)** — the credential model that lets Autopilot run with vendor keys safely.
