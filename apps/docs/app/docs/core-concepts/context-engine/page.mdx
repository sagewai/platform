...
```mermaid
flowchart TD
    SRC["Documents / Text / URLs / Directories"]
    IP["IngestionPipeline"]
    DOC["Docling (PDF, DOCX, HTML, images)"]
    TS["tree-sitter (source code: Python, JS, Go, ...)"]
    URL["URL fetcher (with SSRF protection)"]
    CHUNK["Chunking (recursive, with semantic dedup)"]
    EMB["Embedding (OpenAI, configurable)"]
    VS["Vector Store (Milvus or in-memory)"]
    MS["Metadata Store (Postgres or in-memory)"]
    GS["Graph Store (NebulaGraph, optional)"]
    MSR["Multi-Strategy Retrieval"]
    VSe["Vector search (semantic similarity)"]
    BM["BM25 search (keyword matching)"]
    GSe["Graph search (relationship traversal)"]
    RRF["Reciprocal Rank Fusion (RRF) merge"]
    RE["Optional cross-encoder re-ranking"]
    OUT["Deduplicated, scored results"]
    SRC --> IP
    IP --> DOC & TS & URL
    DOC & TS & URL --> CHUNK --> EMB
    EMB --> VS & MS & GS
    VS & MS & GS --> MSR
    MSR --> VSe & BM & GSe
    VSe & BM & GSe --> RRF --> RE --> OUT
```
