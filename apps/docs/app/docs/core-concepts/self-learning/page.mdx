...
flowchart TD
    UM["User Message"]
    AR["Agent receives message"]
    RET["[Retrieve] Search context + episodes + graph"]
    INJ["Inject relevant memories into prompt"]
    LLM["LLM generates response"]
    LRN["[Learn] MemoryBridge extracts facts (every N turns)"]
    STO["Store facts + build graph + update episodes"]
    RR["Return response to user"]
    UM --> AR --> RET --> INJ --> LLM
    LLM --> LRN --> STO
    LLM --> RR
```
