...
flowchart TD
    Q["User Query"]
    O["ContextEngine / RAGEngine (orchestrator)"]
    V["Vector search (semantic similarity)"]
    B["BM25 search (keyword matching)"]
    G["Graph search (relationship traversal)"]
    RRF["Reciprocal Rank Fusion (merge results)"]
    RE["Optional re-ranking (cross-encoder)"]
    INJ["Retrieved context injected into agent messages"]
    Q --> O
    O --> V & B & G
    V & B & G --> RRF
    RRF --> RE --> INJ
