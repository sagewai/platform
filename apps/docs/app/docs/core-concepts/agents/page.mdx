...
flowchart TD
    UM["User Message"]
    Chat["BaseAgent.chat(message)"]
    BM["Build Messages"]
    IM["Inject Memory Context"]
    IG["Check Input Guardrails"]
    EX["ExecutionStrategy.execute()"]
    CL["_call_llm()"]
    LR["LLM Response"]
    TC{"Has tool_calls?"}
    ET["Execute Tools"]
    RT["Return text response"]
    OG["Check Output Guardrails"]
    RR["Return response text"]
    UM --> Chat --> BM --> IM --> IG --> EX --> CL --> LR --> TC
    TC -->|YES| ET --> CL
    TC -->|NO| RT --> OG --> RR
```
