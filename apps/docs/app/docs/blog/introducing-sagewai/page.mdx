...
This post is the April 2026 launch announcement. If you're new to Sagewai, the [Quickstart](/docs/getting-started) is the fastest way in.
...
See the [SDK quickstart](/docs/getting-started/minimal-setup) for the full hello-world flow.
...
You're responsible for the master encryption key (back it up, rotate it on a schedule that fits your compliance posture) and for choosing which credentials belong in which tier. The [Security guide](/docs/security) walks through the setup.
...
- **[Quickstart (15 min)](/docs/getting-started)** — install the SDK, create an encrypted credential profile, and run an agent inside an isolated sandbox.
- **[Minimal setup](/docs/getting-started/minimal-setup)** — a hello-world agent with no sandbox, no profile, no admin server.
