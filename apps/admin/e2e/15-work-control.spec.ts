...
test.describe('Work Control Console', () => {
  ...
  test('shows project-scoped active Work and canonical pending attention', async ({ page }) => {
    ...
    await expect(page.getByText('Approve production delivery?').first()).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Active Work' })).toBeVisible();
    await expect(page.getByText('READY TO DELIVER').first()).toBeVisible();
    await expect(page.getByRole('link', { name: 'Fleet workers', exact: true })).toHaveAttribute('href', '/fleet');
    ...
    await expect(page.getByText('Failed to load Work control state.', { exact: true })).toBeVisible();
  ...
  test('shows canonical event actors, delivery history, and Evidence Board references', async ({ page }) => {
    ...
    await expect(page.getByText('evidence://observation/fail').first()).toBeVisible();
    await expect(page.getByText('evidence://rollback/safe').first()).toBeVisible();
  });
});
