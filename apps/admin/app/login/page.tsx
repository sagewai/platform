      ...
      <div className="w-[420px] relative z-10">
        ...
        <div className="bg-surface-dark rounded-xl border border-border-dark p-8 shadow-2xl shadow-black/20">
          {connState === 'disconnected' && !error && (
            <div className="flex items-center gap-2.5 bg-warning/10 border border-warning/20 px-3.5 py-2.5 rounded-lg mb-6 text-[13px] text-warning">
              <WifiOff className="w-4 h-4 flex-shrink-0" />
              <span>Backend server is not reachable. Login may fail.</span>
            </div>
          )}
          ...
          <div className="mt-5 text-center text-[13px] text-text-muted">
            <Link href="/forgot-password" className="text-primary no-underline hover:underline">Forgot password?</Link>
          </div>
          <div className="mt-3 text-center text-[13px] text-text-muted">
            Don&apos;t have an account?{' '}
            <Link href="/register" className="text-primary no-underline font-medium hover:underline">Sign up</Link>
          </div>
        </div>
